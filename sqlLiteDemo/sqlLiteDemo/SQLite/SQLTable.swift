...
class SQLTable: NSObject, SQLTableProtocol {
		...
		if verified == nil || !verified! {
			...
			if cnt == 1 {
				...
				if rc == 0 {
                    ...
                    print("creating table - \(table) with SQL: \(sql)")
                }
				SQLTable.verified[table] = true
			} else {
    ...
    class func DeleteAllRowFromMessageModels(isGroup : Bool, strJID : String) -> Bool{
        let db = SQLiteDB.shared
        var params: [Any]? = [true, Date()]
        params = nil
        var sql = ""
        if(isGroup){
          sql = "DELETE FROM \(table) WHERE group_JId = '\(strJID)'"
        }else{
           sql = "DELETE FROM \(table) WHERE friend_jid = '\(strJID)'"
        }
        
        let rc = db.execute(sql:sql, parameters: params)
        return (rc != 0)
    }
    class func DeleteAllRowFromFrequestMessageModel(isGroup : Bool, strJID : String) -> Bool{
        let db = SQLiteDB.shared
        var params: [Any]? = [true, Date()]
        params = nil
        let  sql = "DELETE FROM \(table) WHERE receiver_id = '\(strJID)'"
        let rc = db.execute(sql:sql, parameters: params)
        return (rc != 0)
    }
    class func DeleteAllRowFromGroupMember(strJID : String) -> Bool{
        let db = SQLiteDB.shared
        var params: [Any]? = [true, Date()]
        params = nil
        let  sql = "DELETE FROM \(table) WHERE groupJId = '\(strJID)'"
        let rc = db.execute(sql:sql, parameters: params)
        return (rc != 0)
    }
    class func DeleteGroupMember(strGroupJID : String, strMemberJID : String) -> Bool{
        let db = SQLiteDB.shared
        var params: [Any]? = [true, Date()]
        params = nil
        let  sql = "DELETE FROM \(table) WHERE groupJId = '\(strGroupJID)' AND memberJId = '\(strMemberJID)'"
        let rc = db.execute(sql:sql, parameters: params)
        return (rc != 0)
    }
    class func DeleteAllGroupMemberOrAdmin(strGroupJID : String, strMember: String) -> Bool{
        let db = SQLiteDB.shared
        var params: [Any]? = [true, Date()]
        params = nil
        let  sql = "DELETE FROM \(table) WHERE groupJId = '\(strGroupJID)' AND isAdmin = '\(strMember)'"
        let rc = db.execute(sql:sql, parameters: params)
        return (rc != 0)
    }
    class func DeleteBroadCastList(strBroadCastID : String) -> Bool{
        let db = SQLiteDB.shared
        var params: [Any]? = [true, Date()]
        params = nil
        let  sql = "DELETE FROM \(table) WHERE broadcastJId = '\(strBroadCastID)'"
        let rc = db.execute(sql:sql, parameters: params)
        return (rc != 0)
    }
    class func deleteRow(key:String, id: String) -> Bool {
        let db = SQLiteDB.shared
        let sql = "DELETE FROM \(table) WHERE \(key) = '\(id)'"        
        let rc = db.execute(sql:sql, parameters: nil)
        return (rc != 0)
    }
    class func deleteRow(condition: String) -> Bool {
        let db = SQLiteDB.shared
        let sql = "DELETE FROM \(table) WHERE \(condition)"
        let rc = db.execute(sql:sql, parameters: nil)
        return (rc != 0)
    }
    class func updateAllBooleanValueByKey(key: String, value: Bool) -> Bool {
        let db = SQLiteDB.shared
        let sql = "UPDATE \(table) SET \(key) = \(value)"
        let rc = db.execute(sql:sql, parameters: nil)
        return (rc != 0)
    }
    class func updateValues(setKeyValue: String, condition: String) -> Bool {
        let db = SQLiteDB.shared
        let sql = "UPDATE \(table) SET \(setKeyValue) WHERE \(condition)"
        let rc = db.execute(sql:sql, parameters: nil)
        return (rc != 0)
    }
    
    @discardableResult
    func update(param: [String: Any] = [:]) -> Bool{
        let db = SQLiteDB.shared
        let data = values()
    ...
    class func AddNewMemberInGroup(MemberModer : [String: Any] ) -> Bool{ //GroupMember
...
//    func updateFrequestMessageModels(unreadCount:String = "0",lastMessage:String? = nil,roomId:String) {
//        let key = roomId
//
...
//            sql = "UPDATE \(table) SET recentMessage = '\(lastMessage)' ,unreadMessageCount = '\(unreadCount)'   WHERE room_Id = \(key)"
//        }else if let lastMessage = lastMessage{
//            sql = "UPDATE \(table) SET recentMessage = '\(lastMessage)' WHERE room_Id = \(key)"
//        }else{
//            sql = "UPDATE \(table) SET unreadMessageCount = '\(unreadCount)'   WHERE room_Id = \(key)"
//        }
//        let _ = db.query(sql:sql)
//    }
